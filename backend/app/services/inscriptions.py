...
def parent_remplacer_demande_non_bio_sans_changer_slot(
    *,
    db: Session,
    user: User,
    demande_id: int,
    ...
    """
    Remplacement non codifié (lien AUTRE) sur la même demande:
    conserve strictement liste/rang/date/heure/slot.
    """
    raise_if_liste_finale_definitive()
    parent = db.query(Parent).filter(Parent.user_id == user.id).first()
    if not parent:
    ...
    if demande.statut == DemandeStatut.DESISTEE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Remplacement impossible : demande deja desistee.",
        )
    if demande.desistement is not None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Remplacement impossible : annulez d'abord le desistement en cours.",
        )
    if demande.liste is None or demande.liste.code != ListeCode.ATTENTE_N2:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Remplacement non codifié autorisé uniquement sur la liste N°2.",
        )
    if demande.enfant.lien_parente != LienParente.AUTRE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Remplacement non codifié réservé aux enfants avec lien de parenté « Autre ».",
        )
